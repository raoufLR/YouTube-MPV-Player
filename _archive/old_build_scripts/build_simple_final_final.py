...
try:
    ...
    if result.returncode == 0:
        ...
        if os.path.exists(exe_path):
            ...
            print("  - YouTube video search and playback")
            print("  - MPV player with seek controls")
            print("  - User preferences and history management")
            print("  - Playlists and saved videos")
            print("  - Thumbnail previews")
            sys.exit(0)
        else:
            print("\n[ERROR] Executable not found in dist/ folder")
            print("Build may have created a .exe file with a different name or in a subdirectory")
            sys.exit(1)
    else:
        print("\n[ERROR] Build failed:")
        print(result.stderr)
